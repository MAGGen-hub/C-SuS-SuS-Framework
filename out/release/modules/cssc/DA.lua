local match,format,insert,remove,unpack,error,tostring,setmetatable,native_load,placeholder_func,t_swap=ENV(2,3,7,9,10,14,15,18,20,22,23)
    ...
    for i=1,#da_data,4 do
        ...
        if value==nil and default_arg then insert(res,default_arg)--arg not inited, replace with default
            ...
            if type_check then --type check
                ...
                type_check=type_check==1 and {[l_typeof(default_arg)]=1} or t_swap{(native_load("return "..type_check,"DA_type_loader",nil,def_arg_meta)or placeholder_func)()}--> dynamic type! must be equal to def_arg type OR parce type value
...
Event.reg("lvl_close",function(lvl)-- def_arg injector
    if lvl.DA_d then --level had default_args
        local da_tab,build_arr,name,comma_obj,da_data,obj,strict_type_def,arg_len=lvl.DA_d,{},{},{",",2,Cdata.opts[","][1]}
        for i=da_tab.c_a,1,-1 do --parce args
            if da_tab[i]then --def_arg exist
                da_data,arg_len,strict_type_def=da_tab[i],0

                insert(name,{da_data[4],3}) insert(name,comma_obj)--insert arg name
                
                if not da_data[2] then --no def_arg -> insert nil -> type only
                    insert(build_arr,{"nil",8}) insert(build_arr,comma_obj)
                end 
                ...
                for j=da_data[3],da_data[1]or da_data[2],-1 do --to minimum value
                    ...
                    elseif not skipper_tab[obj[2]] then--strict_type(da_data[1] - 100% exist -> da_data[2] already parced and things to parce still exist)
                        if not(obj[2]==3 or obj[2]==7 or match(obj[1],"^nil")) or strict_type_def then 
                ...
                if arg_len or not da_data[1] then --no strict type inset nil
                    ...
                    insert(build_arr,{arg_len and"1"or "nil",8}) 
                    insert(build_arr,comma_obj) 
                end 

                insert(build_arr,{da_data[4],3}) insert(build_arr,comma_obj) --insert arg name
                insert(build_arr,{tostring(i),8}) insert(build_arr,comma_obj)--insert arg index
            end
        end
