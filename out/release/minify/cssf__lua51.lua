...
local base_path=[[/home/maggen/.local/share/craftos-pc/computer/0/cssf_final/out/release/minify/]]local cssf,A,S,T,E,Cp,E_ENV,_={},assert,string,table,"cssf load failed because of missing libruary method!",function()end
local SG,Sm,SF,Sf,Sg,Ss,Ti,Tc,Tr,Tu,Gt,Gp,Ge,Gs,Gn,GG,GS,GP=A(S.gmatch,E),A(S.match,E),A(S.format,E),A(S.find,E),A(S.gsub,E),A(S.sub,E),A(T.insert,E),A(T.concat,E),A(T.remove,E),A(unpack,E),A(type,E),A(pairs,E),A(error,E),A(tostring,E),A(tonumber,E),A(getmetatable,E),A(setmetatable,E),A(pcall,E)local bit32=GP(require,"bit")and require"bit"or GP(require,"bit32")and require"bit32"or GP(require,"bitop")and(require"bitop".bit or require"bitop".bit32)or print and print"Warning! Bit32/bitop libruary not found! Bitwize operators module disabled!"and nil
if bit32 then
...
local Gl,Gf
do
local loadstring,load,setfenv,loadfile=A(loadstring,E),A(load,E),A(setfenv,E),A(loadfile,E)Gl=function(x,name,mode,env)local r,e=(Gt(x)=="string"and loadstring or load)(x,name)if env and r then setfenv(r,env)end
return r,e
end
Gf=function(filename,mode,env)local r,e=loadfile(filename)if env and r then setfenv(r,env)end
return r,e
end
end
local TC,TS,Ce=function(s,o,f)o=o or{}for k,v in Gp(s)do o[k]=f and o[k]or v end return o end,function(t,o)o=o or{}for k,v in Gp(t)do o[v]=k end return o end,function(...)local r={}for k,v in Gp{...}do Ti(r,E_ENV[v]or false)end return Tu(r)end
...
cssf=GS({Configs={cssc_basic="sys.err,cssc={NF,KS,BO,CA,ncbf}",cssc_user="sys.err,cssc={NF,KS(sc_end),LF,DA,BO,CA,NC,IS,ncbf}",cssc_full="sys.err,cssc={NF,KS(ret,loc,sc_end,pl_cond),LF,DA,BO,CA,NC,IS,ncbf}",lua_53="sys.err,cssc={BO,ncbf}"},creator="M.A.G.Gen.",version='4.6-beta'},{__type="cssf",__name="cssf",__call=function(S,X)if"string"~=Gt(X)then Ge(SF("Bad argument #2 (expected string, got %s)",Gt(X)))end
local i,C,O,r=1
r={__call=function(S,s,...)if#S>999 then Tr(S,1)end
Ti(S,SF("%-16s : "..s,SF("[%0.3d] [%s]",i,S._),...))i=i+1
end}C={load_lib=Cl,load_libs=CL,PostLoad={},PreRun={},PostRun={},Struct={},Loaded={},Clear={},Result={},error=GS({_=" Error "},r),log=GS({_="  Log  "},r),warn=GS({_="Warning"},r),Core=Cp,Iterator=Gl"return 1",}O=GS({run=run,info="C SuS SuS Framework"},{__type="cssf_unit",__name="cssf_unit",__index={data=C}})C.User=O
