...
local base_path=[[/home/maggen/.local/share/craftos-pc/computer/0/cssf_final/out/release/]]local cssf,A,S,T,E,Cp,E_ENV,_={},assert,string,table,"cssf load failed because of missing libruary method!",function()end
local SG,Sm,SF,Sf,Sg,Ss,Ti,Tc,Tr,Tu,Gt,Gp,Ge,Gs,Gn,GG,GS,GP=A(S.gmatch,E),A(S.match,E),A(S.format,E),A(S.find,E),A(S.gsub,E),A(S.sub,E),A(T.insert,E),A(T.concat,E),A(T.remove,E),A(Tu,E),A(type,E),A(pairs,E),A(error,E),A(tostring,E),A(tonumber,E),A(getmetatable,E),A(setmetatable,E),A(pcall,E)local bit32=GP(require,"bit")and require"bit"or GP(require,"bit32")and require"bit32"or GP(require,"bitop")and(require"bitop".bit or require"bitop".bit32)or print and print"Warning! Bit32/bitop libruary not found! Bitwize operators module disabled!"and nil
if bit32 then
...
end}Control={load_lib=load_lib,load_libs=load_libs,PostLoad={},PreRun={},PostRun={},Struct={},Loaded={},Clear={},Result={},Ge=GS({_=" Error "},r),log=GS({_="  Log  "},r),warn=GS({_="Warning"},r),Core=Cp,Iterator=Gl"return 1",}Obj=GS({run=run,info="C SuS SuS Framework"},{__type="cssf_unit",__name="cssf_unit",__index={data=Control}})Control.User=Obj
