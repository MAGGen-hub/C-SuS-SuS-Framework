...
--ARG CHECK FUNC
local arg_check,t_copy,t_swap,Modules,Features=
	function(Control)if(getmetatable(Control)or{}).__type~="cssc_unit"then error(format("Bad argument #1 (expected cssc_unit, got %s)",type(Control)),3)end end,function(s,o,f) for k,v in pairs(s)do o[k]=f and o[k]or v end end,function(t,o)o=o or {}for k,v in pairs(t)do o[v]=k end return o end
--LOCALS
local Configs,_arg,load_lib,continue,clear,make,run,read_control_string,load_control_string=
{lua_mc_basic="sys.err,cssc={NF,KS,LF,BO,CA}",--configs
 lua_mc_user="sys.err,cssc={NF,KS(sc_end),LF,DA,BO,CA,NC,IS}",
 lua_mc_full="sys.err,cssc={NF,KS(sc_end,pl_cond),LF,DA,BO,CA,NC,IS}"},
...
function(Control,path,...)arg_check(Control)--LOAD_LIB function TODO:REWORK
	...
	if false~=ld then
		...
		local r,e=native_loadfile(base_path.."features/"..path..".lua",nil,setmetatable({C=Control,Control=Control,ENV=load_env,_G=_G},{__index=Control}))
		e=e and error(e)--error(format('Error loading libruary "%s": %s',path,e))
		arg={r(Control,...)}
...
--TODO:rework!!
load_control_string=function(Control,main,sub,path,cur_sc)--RECURSIVE FUNC: load readed control string and fill Control table with contents of loaded modules
	local prt,mod,e,sc
	if path then--LOAD MODULE
		prt=remove(main,1)
		--mod=nxt[prt]or{}--TODO:replace with loadfile!!!
		path=path..((cur_sc or{})[prt] or prt)--apply aliases if exists 
		--try_load mofule from file
		mod,e=native_loadfile(base_path.."modules/"..gsub(sub(path,2),"%.","/")..".lua",nil,setmetatable({C=Control,Control=Control,ENV=load_env,_G=_G},{__index=Control}))
	...
	if mod and sc~=1 then  --load sub_modules if exist
		...
		if #main>0 then load_control_string(Control,main,sub,path,sc)
...
Features={code={cdata=local insert,remove,t_swap=ENV(7,9,24)
...
cssc={op_stack=local match,insert,remove,unpack = ENV(2,7,9,10)
...
pdata=local match,format,insert,concat,unpack,pairs,error,setmetatable = ENV(2,3,7,8,10,14,15,19)
local path,dt=...
...
lua={base=local match,pairs,tonumber = ENV(2,14,17)
...
struct=local match,format,gsub,sub,insert,concat,unpack=
ENV(2,3,5,6,7,8,10)
...
syntax_loader=local gmatch,unpack,placeholder_func = ENV(1,10,23)
...
common={event=local insert,remove,type,pairs=ENV(7,9,13,14)
--EVENT SYSTEM
local clr,e
...
insert(Control.Clear,clr)
,
level=local sub,insert,remove,pairs=ENV(6,7,9,14)
local level_hash=...
--LEVELING SYSTEM
...
text={dual_queue={base=local insert,concat=ENV(7,8)
...
iterator=local gmatch,insert=ENV(1,7)
local seq=...
...
make_react=local match,sub,insert=ENV(2,6,7)
...
parcer=local match,sub,type=ENV(2,6,13)
...
space_handler=local match,gsub,insert=ENV(2,5,7)
...
Modules={cssc={BO=local match,format,insert,floor,type,pairs,error,getmetatable,pcall,native_load,bit32,t_swap=ENV(2,3,7,11,13,14,15,18,20,21,22,24)
local mod,arg=...
...
return 1,
CA=local match,insert,unpack,pairs,t_swap = ENV(2,7,10,14,24)
...
return 1,
DA=local match,format,insert,remove,unpack,error,tostring,setmetatable,native_load,placeholder_func,t_swap=ENV(2,3,7,9,10,15,16,19,21,23,24)
...
insert(Control.Clear,function()used = nil end)
return 1,
IS=local insert,type,error,t_swap=ENV(7,13,15,24)
...
insert(Control.Clear,function()used=nil end)
return 1,
KS=local match,sub,pairs=ENV(2,6,14)
...
return 1
,
NC=local sub,insert,pairs,setmetatable,t_swap=ENV(6,7,14,19,24)
...
return 1,
NF=local gmatch,match,sub,insert,concat,tostring,tonumber=ENV(1,2,6,7,16,17)
...
cssc=local match,insert,remove,unpack,placeholder_func = ENV(2,7,9,10,23)
...
,
sys={dbg_hl=local match,gsub,sub,insert,concat,pairs=ENV(2,5,6,7,8,14)
...
return 1,
err=local format,unpack,error = ENV(3,10,15) 
...
lua_mc={make=make,run=run,clear=clear,Configs=Configs}
