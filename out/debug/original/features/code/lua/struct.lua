...
insert(Struct,function()
	...
	if#C.operator>0 then --string structures
		...
		if Lvl then --LONG BUILDER
			Lvl="%]"..Lvl.."()%]"
			repeat
				if split_seq(rez,match(C.operator,Lvl))then mode=comment and 11 or 7 break end --structure finished
				insert(rez,C.word)
			until Iterator()
		elseif str_obj then --STRING BUILDER
			...
			while C.index do
				...
				if split_seq(rez,mode)then--end of string found
					...
					if #comment%2>0 then mode=not mode and 7 break end --end of string or \n found
