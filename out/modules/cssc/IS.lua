...
return r
end)Words["is"]=function()Runtime.reg("__cssc__kw_is","kwrd.is")local i,d=Cd.tb_while(S)if c[d[1]]then C.Ge("Unexpected 'is' a '%s'!",Result[i])end
Cssc.inject(",",2,Cd.opts["^"][1])Text.split_seq(nil,2,1)Event.run(2,"is",2,1)Event.run("all","is",2,1)Event.reg("all",function(obj,tp)if a[tp]or tp==2 and not Cd[#Cd][3]then C.Ge("Unexpected '%s' a 'is'!",obj)end
