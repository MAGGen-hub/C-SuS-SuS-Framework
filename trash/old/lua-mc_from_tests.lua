...
if not bit32 then
    local check = false
    check, bit32=pcall(require,"bit32")
    if not check then
        print("Warning! Bit32 libruary not found! Bitwize operators module disabled!")
	bit32 = false
    end
end
...
local native_load
if find(_HOST or "","ComputerCraft") then --we are in CraftOS
   native_load = load
elseif _VERSION=="Lua5.2" then
   native_load = load    -- native load function from curent Lua environment
else
   local loadstring = loadstring
   local load = load
   local setfenv = setfenv
   native_load = function(x,name,mode,env)
	local result,err = (type(x)=="string" and loadstring or load)(x,name)
	if env and result then
		setfenv(result,env)
	end
	return result,err
   end
...
lua_mc.load = function (x,name,mode,env)
...
err= function (Ctrl,err_msg)
    Ctrl.err=Ctrl.err or "LuaMC["..Ctrl.line.."]:"..err_msg
end
...
Features.pre={ function (Ctrl,script_id,x,name,mode,env)
     if  not lua_mc.preload then lua_mc.preload={} end 
     local P=lua_mc.preload
...
ATP= function (Ctrl,wt,on)err(Ctrl,"Attempt to perform '"..wt.."' on '"..on.."'!") end 
 ...
 --LANGUAGE CORE
Features.Lang_Core={ function (Ctrl)
    ...
    Ctrl.Finaliser[1]= function (x,n,m,e)
         ...
         --if p then p.path=p.path..Lang.path --if package.path exist then add additional path
...
for v,k in gmatch('.:"({',"()(.)")do
	...
	if find(sub(operator,3),"[^%z%s]") then -- %z for \0
		ATP(Ctrl,OBJ(sub(operator,3)),k) end 
        insert(r,Ctrl.Level[#Ctrl.Level].start_of_object," lua_mc.nilF(") --first part
...
Features.CA={ function (Ctrl)
    Features.Lang_Core[1](Ctrl) --load start searcher
    ...
    local op= function (Ctrl,operator,word)
        ...
        r[#r+1]="="..(operator=="?" and "lua_mc.q_eq(" or "") --insert equality or ?=
...
Features.M={ function (Ctrl)
    ...
    Features.Lang_Core[1](Ctrl) --enable core
    local l=Ctrl.Level
    local r=Ctrl.Result
    l[#l].m={bor=1}
     --on level open
        ...
        Ctrl.Operator[v]= function (Ctrl,operator,word)
            ...
            if v=='~' and  --posible unary operator
                ...
                keywords_ids[p] or kp[p]) then  -- there is no keyword before op
                    r[#r+1]="lua_mc.mt.bnot^"  return 
            end     --bnot located. Insert and return...
            
            insert(r,l[#l].m[k] or l[#l].m.bor or l[#l].start_of_object,"lua_mc.setmetatable({")
            r[#r+1]="},lua_mc.mt."..k..(v=='//' and ')/' or ')..')
            local i=#r+1
...
lua_mc.setmetatable=setmetatable
lua_mc.features=Features
lua_mc.nilF=NIL_Forgive
lua_mc.mt=M
lua_mc.version="4.1-alpha"
lua_mc.creator="M.A.G.Gen."
lua_mc._ENV=_ENV
lua_mc.is=is
lua_mc.q_eq=qeq
