...
local comp1 = lua_mc.make("cssc={BO}")--basic compiller
...
local comp2 = lua_mc.make("cssc={KS,LF,BO,CA}")
compilled=comp2:run(lzss_src_oprimised)--compile advanced lzss
print(compilled)

local lzss_advanced = comp1:cssc_load("basic_lzss",nil,setmetatable({},{__index=_G}))()--specific _ENV with _G protected

assert(lzss_advanced,"Unable to compile advanced lzss src")

local compressed_lzss_src=lzss_advanced.compress(lzss_src)
